...
const toFinite = (n: number, fallback = 0) => (Number.isFinite(n) ? n : fallback)
...
export const parseOKLCH = (s: string): Oklch => {
	...
	const m = s.match(/oklch\(\s*([^\s,/]+)\s+([^\s,/]+)\s+([^\s,/\)]+)(?:\s*\/[^\)]*)?\)/i)
	if (m) {
		return {
			l: clamp01(parseFloat(m[1] || '0') || 0),
			c: Math.max(0, parseFloat(m[2] || '0') || 0),
			h: (((parseFloat(m[3] || '0') || 0) % 360) + 360) % 360
		}
	}
	return { l: 0, c: 0, h: 0 }
}
...
const toHexFromOKLCH = (oklch: Oklch): string => {
	try {
		...
		const h = ((toFinite(oklch.h, 0) % 360) + 360) % 360
...
const toHexFromString = (s: string): string => {
	try {
		...
		return normalizeHex(new Color(s).to('srgb').toString({ format: 'hex' }))
	} catch {
		return '#000000'
	}
}
...
export function OklchColorPicker({
	...
	const externalHex = React.useMemo(() => toHexFromOKLCH(value), [value.l, value.c, value.h])
	...
	const handleHexChange = React.useCallback(
		(next: string) => {
			...
			if (onChange) {
				if (rafRef.current !== null) cancelAnimationFrame(rafRef.current)
				rafRef.current = requestAnimationFrame(() => onChange(o))
			}
			if (onChangeCommitted) {
				if (commitTimeoutRef.current !== null) clearTimeout(commitTimeoutRef.current)
				commitTimeoutRef.current = setTimeout(() => onChangeCommitted(o), debounceMs)
			}
		},
	...
	return (
		<div className={className}>
			<HexColorPicker className="!w-full" color={hex} onChange={handleHexChange} />
			{withInput && (
				<div className="mt-2">
					<HexColorInput
						color={hex}
