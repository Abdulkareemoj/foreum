...
export default function LeftSidebar() {
  ...
  const { data: trendingTags, isLoading: tagLoading } =
    trpc.tag.popular.useQuery({ limit: 10 });
  const { data: topContributors, isLoading: userLoading } =
    trpc.user.topContributors.useQuery({ limit: 5 });
  const { data: activeDiscussions, isLoading: discLoading } =
    trpc.thread.recent.useQuery({ limit: 5 });
                ...
                >
                  <div className="flex items-center justify-between gap-5 rounded-lg p-2 hover:bg-accent">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {disc.replies || disc.replyCount || 0} replies
                    </span>
                  </div>
                </Link>
              ))}
