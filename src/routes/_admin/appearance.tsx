...
import {
  ThemeDataProvider,
  useThemeData,
} from "~/providers/theme-data-provider";
...
import { PresetBrowserDialog } from "~/components/admin/theme-options/preset-browser-dialog";
import { SaveToAccountButton } from "~/components/admin/theme-options/save-to-account-button";
import { usePresetBrowserStore } from "~/stores/preset-browser-store";
import { Separator } from "~/components/ui/separator";
import { Avatar, AvatarFallback } from "~/components/ui/avatar";
import { Sun, Moon, RotateCcw, CheckCircle, Bookmark, Save } from "lucide-react";
    ...
    >
      ...
      <div className="flex flex-col gap-4">
        ...
        {[
          ...
          {
            title: "Best practices for this theme",
            tag: "General",
            replies: 4,
          },
          {
...
function AppearanceToolbar() {
  const { previewMode, setPreviewMode, isSaving, resetTheme, theme, themeName } = useThemeData();
  const setPresetOpen = usePresetBrowserStore((s) => s.setOpen);
  const utils = trpc.useUtils();
  const savePreset = trpc.theme.createPreset.useMutation({
    onSuccess: () => {
  ...
  function handleSavePreset() {
    if (!theme) return;
    savePreset.mutate({ name: themeName || "Untitled Theme", data: { ...theme, name: themeName || "Untitled Theme" } });
  }
  ...
  return (
    <div className="flex items-center justify-between px-4 py-2 border-b bg-card shrink-0">
      <div className="flex items-center gap-2">
        ...
        {!isSaving && (
          <span className="flex items-center gap-1 text-xs text-muted-foreground">
            <CheckCircle className="text-green-500" /> Auto-saved
          </span>
        )}
      </div>
      <div className="flex items-center gap-2">
        <Button size="sm" variant="outline" onClick={handleSavePreset} disabled={savePreset.isPending || !theme}>
          {savePreset.isPending ? <Spinner /> : <Save />}
          Save
        </Button>
        <SaveToAccountButton />
        ...
        <div className="w-72 shrink-0 border-r flex flex-col bg-card h-full">
          <div className="px-4 py-3 border-b shrink-0 space-y-1">
            ...
            <p className="text-xs text-muted-foreground">
              Customise your forum's look and feel. Changes are saved
              automatically.
            </p>
          </div>
